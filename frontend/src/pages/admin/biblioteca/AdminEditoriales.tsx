...
import {
  useListarEditoriales,
  useActivarEditorial,
  useDesactivarEditorial,
} from "@/hooks";
import { EditorialFormModal } from "@/components/biblioteca/EditorialFormModal";
...
import type { EditorialResponse } from "@/api/types";

type Tab = "all" | "activos" | "inactivos";
const PAGE_SIZE = 20;

export default function AdminEditoriales() {
  const [tab, setTab] = useState<Tab>("all");
  const [page, setPage] = useState(1);
  const [modalAbierto, setModalAbierto] = useState(false);
  const [editorialEditando, setEditorialEditando] = useState<EditorialResponse | null>(null);
  const params = { skip: (page - 1) * PAGE_SIZE, limit: PAGE_SIZE };
  const todos = useListarEditoriales("all", params);
  const activos = useListarEditoriales("activos", params);
  const inactivos = useListarEditoriales("inactivos", params);
  const activar = useActivarEditorial();
  const desactivar = useDesactivarEditorial();

  const current = tab === "all" ? todos : tab === "activos" ? activos : inactivos;

  const tabs: { key: Tab; label: string; count: number }[] = [
    { key: "all", label: "Todas", count: todos.data?.total ?? 0 },
    { key: "activos", label: "Activas", count: activos.data?.total ?? 0 },
    { key: "inactivos", label: "Inactivas", count: inactivos.data?.total ?? 0 },
  ];
  ...
  const abrirNueva = () => {
    setEditorialEditando(null);
    setModalAbierto(true);
  };

  const abrirEdicion = (editorial: EditorialResponse) => {
    setEditorialEditando(editorial);
    setModalAbierto(true);
  };

  const editoriales = current.data?.items ?? [];
  ...
  if (current.isError) {
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 p-6 text-center">
        <p className="text-sm text-red-600">Error al cargar las editoriales.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#18181B]">Editoriales</h1>
          <p className="text-sm text-[#52525B]">
            Gestión de editoriales que publican los libros.
          </p>
        </div>
        <button
          onClick={abrirNueva}
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-[#E4E4E1] bg-white p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#A1A19A]">Activas</p>
          </div>
        </div>
        <div className="rounded-xl border border-[#E4E4E1] bg-white p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#A1A19A]">Inactivas</p>
          </div>
        </div>
      </div>
      ...
      {editoriales.length === 0 ? (
        <div className="rounded-lg border border-[#E4E4E1] bg-white p-12 text-center">
          <Building2 className="mx-auto mb-3 h-10 w-10 text-[#E4E4E1]" />
          <h3 className="text-lg font-medium text-[#18181B]">No hay editoriales</h3>
          <p className="mt-1 text-sm text-[#52525B]">No hay editoriales en esta categoría.</p>
        </div>
      ) : (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-lg bg-[#EFF4FE] border border-[#BFDBFE] flex items-center justify-center text-sm font-bold text-[#2563EB] shrink-0">
                        {ed.nombre.charAt(0).toUpperCase()}
                      </div>
                      <div className="min-w-0">
                        <p className="font-medium text-[#18181B] truncate">{ed.nombre}</p>
                        <p className="text-xs text-[#A1A19A] truncate max-w-xs">
                          {ed.descripcion || "Sin descripción"}
                        </p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <ActivoBadge activo={ed.activo} />
                  </td>
                  <td className="px-4 py-3">
                    <div
                    ...
                    >
                      ...
                      {ed.activo ? (
                        <button
                          onClick={() => desactivar.mutate(ed.id)}
      ...
      {totalPaginas > 1 && (
        <div className="flex items-center justify-center gap-2">
          ...
          <span className="text-sm text-[#52525B]">
            Página {page} de {totalPaginas} ({total} editoriales)
          </span>
          <button
