...
import { useCrearEditorial, useActualizarEditorial } from "@/hooks";
import { X } from "lucide-react";
import type { EditorialResponse } from "@/api/types";
import { extraerMensajeError } from "@/lib/errors";

interface EditorialFormModalProps {
  abierto: boolean;
  editorial: EditorialResponse | null;
  onClose: () => void;
}

export function EditorialFormModal({ abierto, editorial, onClose }: EditorialFormModalProps) {
  const crear = useCrearEditorial();
  const actualizar = useActualizarEditorial();
  const [nombre, setNombre] = useState(editorial?.nombre ?? "");
  const [descripcion, setDescripcion] = useState(editorial?.descripcion ?? "");

  const prevId = useRef(editorial?.id);
  useEffect(() => {
    if (editorial?.id !== prevId.current) {
      setNombre(editorial?.nombre ?? "");
      setDescripcion(editorial?.descripcion ?? "");
      prevId.current = editorial?.id;
    }
  }, [editorial]);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (editorial) {
      actualizar.mutate({ id: editorial.id, data: payload }, { onSuccess: onClose });
    } else {
      ...
      <div className="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-[#18181B]">
            {editorial ? "Editar editorial" : "Nueva editorial"}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              {isPending ? "Guardando..." : editorial ? "Guardar cambios" : "Crear editorial"}
            </button>
          </div>
        </form>
      </div>
